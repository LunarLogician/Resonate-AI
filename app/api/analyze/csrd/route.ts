...
import csrd_checklist from '@/public/data/csrd_checklist.json';
...
export async function POST(req: Request) {
  ...
  try {
    const checklist = csrd_checklist;
    ...
    const grouped: Record<string, { section: string; question: string; best: number; topChunk: string; threshold: number }> = {};
    ...
    const final = await Promise.all(
      Object.entries(grouped).map(async ([id, entry]) => {
        const threshold = entry.threshold ?? 0.60;
        const similarity = entry.best;
    
        // ✅ Normalize similarity to match score percentage
        ...
        if (status !== '✅ Likely Met') {
          const chat = await openai.chat.completions.create({
            ...
            messages: [
              {
                role: 'system',
                content: `You are a sustainability reporting advisor. Provide clear, actionable advice (2–3 sentences) to help a company improve its CSRD disclosure to meet the following guideline.`,
              },
              {
                role: 'user',
                content: `CSRD Guideline: "${entry.question}"\n\nDocument Excerpt:\n"${entry.topChunk.slice(
                  0,
                  1000
